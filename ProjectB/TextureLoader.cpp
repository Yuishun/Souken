...
bool CTextureLoader::shadow = false;
...
//      S_OK : succeeded
//
HRESULT CTextureLoader::CreateD2D1BitmapFromFile(ID2D1RenderTarget *pRenderTarget, const TCHAR *pFilename, ID2D1Bitmap **ppBitmap) {
	HRESULT hr;
		...
		if (SUCCEEDED(hr)) {
			...
			for (int row = 0; row < height; ++row) {
				...
				for (int col = 0; col < width; ++col) {
					a = frac * p[3];
					if (CTextureLoader::shadow == false) {
						p[0] = (BYTE)(a * p[0]);
						p[1] = (BYTE)(a * p[1]);
						p[2] = (BYTE)(a * p[2]);
					}
...
//      S_OK : succeeded
//
HRESULT CTextureLoader::CreateTextureFromFile(ID3D11DeviceContext *pContext, const TCHAR *pFilename, ID3D11Texture2D **ppTexture, DWORD *pSrcWidth, DWORD *pSrcHeight, FillMode dwFillmode){
	IWICBitmapDecoder *pDecoder = NULL;
	IWICBitmapFrameDecode *pFrame = NULL;
	WICPixelFormatGUID pixelFormat;
